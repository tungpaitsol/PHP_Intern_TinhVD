...
$m=$arDate["mon"];
	...
	for($i=1;$i<$d+1;$i++){
		$day=$y."-".$m."-".$i;
		$da=$m."-".$i;
		if(third($day) || in_array($da,$holidays)){
			$holiday++;
		}
	}
	return $d-$holiday;
...
function listWorkTimeF($listWorkTime,$listMemberFullTime,$dataDate){
	...
	$m=$arDate["mon"];
	$ar_code=getMemberCode($listMemberFullTime); //lay ra ma code cua member full time
	$aa=0;
	$ar_code_key=[];
	$ar_member=[];
	...
	foreach ($listMemberFullTime as $key ) {
		...
		foreach ($ar_code_key[$key["code"]] as $key1) {
			...
			$m2=$arDate2["mon"];
			if($m==$m2){
				$time2=getTime($listWorkTime[$key1]["start_datetime"]); //gio bat dau den lam
			$time3=getTime($listWorkTime[$key1]["end_datetime"]); //gio ra ve
			
			if (strtotime($worktime)-strtotime($time2)<0){ //di muon
				$t_s=$time2;
				$time=hourWorkDay($time2,$time3)-1.5;
			}
			else{ // di som
				$t_s=$worktime;
				$time=hourWorkDay($worktime,$time3)-1.5;
			}
...
function listWorkTimeP($listWorkTime,$listMemberPartTime,$dataDate){
	$arDate= getdate(strtotime($dataDate));
	$m=$arDate["mon"];
	$ar_code=getMemberCode($listMemberPartTime); //lay ra ma code cua member part time
	$aa=0;
	$ar_code_key=[];
	$ar_member=[];
	foreach ($ar_code as $key1) {
	...
	foreach ($listMemberPartTime as $key ) {
		...
		foreach ($ar_code_key[$key["code"]] as $key1) {
			...
			if (strtotime($worktime)-strtotime($time2)<0){ //di muon
				$t_s=$time2;
				$time=hourWorkDay($time2,$time3);
			}
			else{ // di som
				$t_s=$worktime;
				$time=hourWorkDay($worktime,$time3);
			}
			if($time>=$key['work_hour']){
				$day+=0.5;
			}
...
function moneyInMonthF($typeWorkTime, $listWorkTime,$date){
	...
	$ar= listWorkTimeF($listWorkTime,$typeWorkTime,$date);
	$i=0;
	foreach ($typeWorkTime as $key) {
		$workdays= $ar[$i]["workdays"];
		$salary=$workdays*($key['salary']/dayWorkInMonth($date));
...
$ar_full_time=moneyInMonthF($listMemberFullTime, $listWorkTime,"2019-04");
function moneyInMonthP($typeWorkTime, $listWorkTime,$date){
	$ar_work_time=$typeWorkTime;
	$ar= listWorkTimeP($listWorkTime,$typeWorkTime,$date);
	$i=0;
	foreach ($typeWorkTime as $key) {
		$workdays= $ar[$i]["workdays"];
		$salary=$workdays*($key['salary']/dayWorkInMonth($date));
...
$ar_part_time=moneyInMonthP($listMemberPartTime, $listWorkTime,"2019-04");
$memberFullTime=[];
