...
if(isset($_POST["totalh"])){ 
	show(sortSumA_Z($data,"order"));
}
//============================click 
if(isset($_POST["totall"])){ 
	show(sortSumZ_A($data));
}
//==============function show
function show($array_sort){
	...
	foreach ($array_sort as $key) { 
		echo('<tr><td>'.$key["id"].'</td><td>'.$key["name"].'</td><td>'.$key["price"].'</td><td>'.$key["quantity"].'</td><td>'.$key["order"].'</td><td>'.$key["price"]*$key["quantity"].'</td></tr>');
	}
	echo '</table>';
}
//=============sap xem tong
